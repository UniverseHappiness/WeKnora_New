...
start_docker() {
	...
	if [ "$NO_PULL" = true ]; then
		...
		# 拉取最新镜像
		log_info "拉取最新镜像..."
		PLATFORM=$PLATFORM "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f docker-compose.dev.yml  up -d
	fi
    if [ $? -ne 0 ]; then
...
stop_docker() {
    ...
    # 停止所有容器
	"$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f docker-compose.dev.yml down --remove-orphans
    if [ $? -ne 0 ]; then
...
if [ "$STOP_SERVICES" = true ]; then
    ...
    if [ "$START_OLLAMA" = true ] && [ "$START_DOCKER" = true ]; then
        if [ $OLLAMA_RESULT -eq 0 ] && [ $DOCKER_RESULT -eq 0 ]; then
            ...
            "$DOCKER_COMPOSE_BIN" $DOCKER_COMPOSE_SUBCMD -f docker-compose.dev.yml logs app-dev docreader postgres --since=10s -f
        else
